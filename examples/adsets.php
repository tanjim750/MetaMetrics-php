...
switch ($mode) {
    case 'all':
        $path = sprintf('/%s/%s/adsets', $config->apiVersion(), $config->adAccountId());
        break;
    ...
    case 'find':
        $path = resourcePath($config, $resourceId, 'Ad Set');
        break;

    default:
        fwrite(STDERR, "Usage:\n");
        fwrite(STDERR, "  php examples/adsets.php all\n");
        fwrite(STDERR, "  php examples/adsets.php campaign <campaign-id>\n");
        fwrite(STDERR, "  php examples/adsets.php find <ad-set-id>\n");
        exit(1);
}
...
function collectionPath(MetaConfig $config, ?string $id, string $label): string
{
    ...
    return sprintf('/%s/%s/adsets', $config->apiVersion(), rawurlencode($id));
}
